...
if __name__ == "__main__":
    ...
    testdataloader = torch.utils.data.DataLoader(
        test_dataset, batch_size=batch_size, shuffle=True, num_workers=int(opt.workers)
    )
    ...
    for i, data in enumerate(testdataloader, 0):
        ...
        heat_feat = (heat_feat - np.min(heat_feat, axis=0)) / (
            np.max(heat_feat, axis=0) - np.min(heat_feat, axis=0)
        )
        color_heat_feat = cv2.applyColorMap(
            (heat_feat * 255).astype(np.uint8), cv2.COLORMAP_JET
        )  # BGR
        ...
        for i in range(batch_size):
            ...
            # save to {i}.png
            plt.savefig(f"{opt.expf}/cls_{feat_dim}D/heatmap_{i}.png")
            # plt.show()
