...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, prev_h, Wx, Wh, _, _, next_h = cache
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for i in range(T):
        next_h, stage_cache = rnn_step_forward(x[:, i, :], prev_h, Wx, Wh, b)
        prev_h = next_h
        h[:, i, :] = next_h
...
def rnn_backward(dh, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, T, H = dh.shape
    D = cache[0][0].shape[1]  # cache[0][0]: x with shape (N, D)
    ...
    for i in range(T - 1, -1, -1):
        ...
        dx[:, i, :] = dx_i
        dWx, dWh, db = dWx + dWx_i, dWh + dWh_i, db + db_i
    dh0 = dprev_h
