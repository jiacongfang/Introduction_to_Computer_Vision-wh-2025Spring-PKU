...
base_scale = np.sqrt(
    ((mesh.vertices.max(axis=0) - mesh.vertices.min(axis=0)) ** 2).sum()
)
...
camera = pyrender.PerspectiveCamera(
    yfov=np.deg2rad(60), aspectRatio=pw / ph, znear=0.1, zfar=10
)
scene.add(camera, camera_pose)

# render
r = pyrender.OffscreenRenderer(pw, ph)
seg_img, depth = r.render(
    scene,
    flags=pyrender.constants.RenderFlags.SEG,
    seg_node_map={obj_node: [255, 0, 0]},
)
